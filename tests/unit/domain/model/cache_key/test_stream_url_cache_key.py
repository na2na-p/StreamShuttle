...
class TestStreamUrlCacheKey:
    ...
    @pytest.mark.parametrize(
        "video_id_value, use_hls, expected_key",
        [
            pytest.param(
                "dQw4w9WgXcQ",
                False,
                "dQw4w9WgXcQ:hls:False",
                id="正常系: use_hls=Falseでキャッシュキーが正しく生成される",
            ),
            pytest.param(
                "dQw4w9WgXcQ",
                True,
                "dQw4w9WgXcQ:hls:True",
                id="正常系: use_hls=Trueでキャッシュキーが正しく生成される",
            ),
            pytest.param(
                "abc_def-123",
                False,
    ...
    def test_value_property_returns_correct_cache_key(
        self, video_id_value: str, use_hls: bool, expected_key: str
    ) -> None:
    ...
    def test_str_returns_cache_key_value(self) -> None:
        ...
        cache_key = StreamUrlCacheKey(_video_id=video_id, _use_hls=False)
    ...
    def test_immutability(self) -> None:
        ...
        with pytest.raises(AttributeError):
            cache_key._use_hls = True  # type: ignore

    def test_equality_same_values(self) -> None:
        """同じ値を持つStreamUrlCacheKeyが等しいことを検証"""
        video_id1 = VideoId(_value="dQw4w9WgXcQ")
        video_id2 = VideoId(_value="dQw4w9WgXcQ")
        cache_key1 = StreamUrlCacheKey(_video_id=video_id1, _use_hls=True)
        cache_key2 = StreamUrlCacheKey(_video_id=video_id2, _use_hls=True)

        assert cache_key1 == cache_key2

    def test_equality_different_use_hls(self) -> None:
        """use_hlsが異なる場合にStreamUrlCacheKeyが異なることを検証"""
        video_id = VideoId(_value="dQw4w9WgXcQ")
        cache_key1 = StreamUrlCacheKey(_video_id=video_id, _use_hls=True)
        cache_key2 = StreamUrlCacheKey(_video_id=video_id, _use_hls=False)
    ...
    def test_equality_different_video_id(self) -> None:
        ...
        cache_key1 = StreamUrlCacheKey(_video_id=video_id1, _use_hls=False)
        cache_key2 = StreamUrlCacheKey(_video_id=video_id2, _use_hls=False)
